...
// ---- bottle overlay: pipes live in jsfs ------------------------------------
//
// os/exec builds child stdio from os.Pipe, which needs syscall.Pipe. jsfs
// provides the pipe (a shared byte queue with a read and a write fd); the Go
// side only remembers which fds are pipes so Read/Write/Close skip fdToFile
// (a pipe fd has no jsFile) and call jsfs directly. A pipe read on an empty
// queue blocks via the same callback path fsCall uses for every fs call.
var (
	pipeFDMu sync.Mutex
	pipeFDs  = map[int]bool{}
)
...
func Pipe(fd []int) error {
	...
	res, err := pipeCreate() // jsfs helper (synchronous): [readFd, writeFd]
	if err != nil {
		return err
	}
	r := res.Index(0).Int()
...
// pipeCreate calls jsfs's synchronous fs.pipe() (no callback) and returns the
// [readFd, writeFd] array.
func pipeCreate() (js.Value, error) {
	if jsFS.Get("pipe").Type() != js.TypeFunction {
		return js.Value{}, ENOSYS
	}
	return jsFS.Call("pipe"), nil
}
